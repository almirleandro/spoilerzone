...
export default function Search() {
  ...
  const movieGrid = movieArray.map(item => {
    return <div className='movieBlock' key={item.id}>
      ...
      <div className='rightSide'>
        ...
        <p><b>Título original: </b>{item.original_title}</p>
  ...
  return <div className='Search'>
    <main className='movieGrid'>{movieGrid}</main><br/>
    ...
    <p style={{textAlign: 'center'}}>{'Página ' + searchResult.page + ' de ' + searchResult.total_pages}</p>
  </div>
}